...
const weekTpl = (document.getElementById("weekTpl") as HTMLTemplateElement)
  .content.firstElementChild;
const hourTpl = (document.getElementById("hourTpl") as HTMLTemplateElement)
  .content.firstElementChild;
const entryTpl = (document.getElementById("entryTpl") as HTMLTemplateElement)
  .content.firstElementChild;
const monitorTpl = (
  document.getElementById("monitorTpl") as HTMLTemplateElement
).content.firstElementChild;

function onSelectionChange(ev: Event): void {
  document.querySelectorAll("select").forEach((element: HTMLSelectElement) => {
    if (element.id !== (ev.target as HTMLElement).id) {
      element.value = "0";
    }
  });
  refreshSchedule();
...
function refreshSchedule() {
  ...
  ["A", "B"].forEach((week) => {
    const weekEle = weekTpl.cloneNode(true) as HTMLElement;
    ...
    for (let hour = 1; hour <= 10; ++hour) {
      const hourEle = hourTpl.cloneNode(true) as HTMLElement;
      ...
      schedule.forEach((entry) => {
        if (entry.hour !== hour) return;
        if (entry.week !== "" && entry.week !== week) return;
        if (classId !== 0 && entry.classId !== classId) return;
        if (teacherId !== 0 && entry.teacherId !== teacherId) return;
        if (roomId !== 0 && entry.roomId !== roomId) return;
        if (subjectId !== 0 && entry.subject !== subjects[subjectId]) return;
...
function showAvailableTeachers(hourEle: HTMLElement, week: string) {
  ...
  for (let day = 1; day <= 5; ++day) {
    ...
    schedule.forEach((entry) => {
      if (entry.hour !== hour && entry.hour !== hour + 1) return;
      if (entry.week !== "" && entry.week !== week) return;
      if (entry.day !== day) return;
      if (!entry.teacherId) return;
      if (possibleTeachers.indexOf(entry.teacherId) !== -1) {
...
function showAvailableRooms(hourEle: HTMLElement, week: string) {
...
function insertEntry(hourEle: HTMLElement, entry: any) {
  const entryEle = entryTpl.cloneNode(true) as HTMLElement;
...
function joinHoursToBlocks(weekEle: HTMLElement) {
  for (let hour = 1; hour <= 10; hour += 2) {
    for (let day = 1; day <= 5; day++) {
      const tdOdd = weekEle.querySelector(
        `td[data-time="${day}-${hour}"]`
      ) as HTMLTableCellElement;
      const tdEven = weekEle.querySelector(
        `td[data-time="${day}-${hour + 1}"]`
      ) as HTMLTableCellElement;
...
function insertMonitors(weekEle: HTMLElement, teacherId: number) {
  const week = weekEle.dataset.week;
  monitors.forEach((entry) => {
    if (
      entry[KEY_MONITOR_TEACHER] !== teacherId ||
      entry[KEY_MONITOR_WEEK] !== week
    ) {
      return;
    }

    const monitorEle = monitorTpl.cloneNode(true) as HTMLElement;
    monitorEle.firstElementChild.textContent = entry[KEY_MONITOR_PLACE];
    weekEle
      .querySelector(
        `[data-time="${entry[KEY_MONITOR_DAY]}-${entry[KEY_MONITOR_BEFORE_HOUR]}"]`
      )
      .prepend(monitorEle);
  });
}
 ...
 */
function deleteEmptyRowsAndWeeks(weekEle: HTMLElement) {
  for (let hour = 9; hour >= 1; hour -= 2) {
    const div = weekEle.querySelector(
      `tbody tr:nth-child(${hour}) td:not(:first-child) div, tbody tr:nth-child(${
        hour + 1
